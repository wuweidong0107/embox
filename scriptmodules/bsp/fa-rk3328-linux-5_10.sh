...
emb_module_id="fa-rk3328-linux-5_10"
...
function depends_fa-rk3328-linux-5_10() {
    emb_callModule fa-toolchain
}

function sources_fa-rk3328-linux-5_10() {
    gitPullOrClone "$md_build" https://github.com/friendlyarm/kernel-rockchip nanopi-r2-v5.10.y-tft
    gitPullOrClone "${md_build}/sd-fuse_rk3328" https://github.com/friendlyarm/sd-fuse_rk3328 kernel-v5.4.y
}

function build_fa-rk3328-linux-5_10() {
...
function _install_module_fa-rk3328-linux-5_10() {
...
function install_fa-rk3328-linux-5_10() {
    local target=$1
    ...
    case "${target}" in
        "image")
            ...
            ;;
        "module")
            _install_module_fa-rk3328-linux-5_10
            ;;
        *)
            _install_module_fa-rk3328-linux-5_10
            md_ret_files=(
...
function upgrade_tf_fa-rk3328-linux-5_10() {
    local dev="$1"
...
function upgrade_usb_fa-rk3328-linux-5_10() {
    # TODO
    echo "unsupported yet"
}

function upgrade_ssh_fa-rk3328-linux-5_10() {
    local ip="$1"
    ...
    case "${target}" in
        ...
        *)
            scp -r ${md_inst}/lib root@${ip}:/
            scp ${data[*]} ${dest}
            echo -e "\nRun command on RK3328:\n $ embox_packages.${md_id} 104 upgrade_tf /dev/mmcblkX"
            ;;
    esac
}