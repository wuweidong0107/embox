...
function install_fa-toolchain() {
    cd gcc-x64
    cat toolchain-6.4-aarch64.tar.gz* | tar xz -C "$md_inst"
    cat toolchain-4.9.3-armhf.tar.gz* | tar xz -C "$md_inst"
    cd ->/dev/null

    cd gcc
    extract arm-linux-gcc-4.4.3.tar.gz "$md_inst"
    extract arm-linux-gcc-4.5.1-v6-vfp.tar.xz "$md_inst"
    cd ->/dev/null
}